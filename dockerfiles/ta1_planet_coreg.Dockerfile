...
# Installing specific (older) version of scikit-image as seems like
# there's a change in newer versions of
# skimage.registration.phase_cross_correlation where registration
# isn't getting enough GCPs to perform registration
RUN conda activate watch && \
    pip install --no-input scikit-image==0.17.2
