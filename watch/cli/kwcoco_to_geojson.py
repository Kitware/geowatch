...
def coco_create_observation(coco_dset, anns, with_properties=True):
    ...
    def combined_properties(properties_list, geometry_list):
        ...
        for cache in properties_list['cache']:
...
def coco_track_to_site(coco_dset, trackid, region_id, site_idx=None,
    ...
    #     features[i]['properties'].setdefault('cache', {})
    #     features[i]['properties']['cache']['trackid'] = trackid
...
def predict_phase_changes(site_id, features):
    ...
    Example:
        ...
        >>> features[-1]['properties']['cache'] = {'phase_transition_days': [100]}
    ...
    if as_summary:
        ...
        properties.update(
            **{
                'type': 'site',
                'region_id': region_id,
                **predict_phase_changes(site_id, features), 'cache': {}
            })
...
def add_site_summary_to_kwcoco(possible_summaries,
    ...
    sitesum_model = RegionModel(features=[t[1] for t in site_summaries])
