...
def run_dino_sv(config):
    ...
    if len(ingress_kwcoco_data.get('videos', ())) > 0:
        ...
        subprocess.run(dino_box_detector.command(), check=True)
        ...
        output_region_model = os.path.join(region_models_outdir,
                                           '{}.geojson'.format(region_id))
        dino_building_filter = DinoBuildingFilter.configure({
        ...
        subprocess.run(dino_building_filter.command(), check=True)
