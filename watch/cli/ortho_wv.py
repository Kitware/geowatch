...
def ortho_wv(stac_catalog, outdir, jobs=1, as_cog=False, te_dems=False, pansharpen=False, gsd=None):
    ...
    orthorectified_catalog = parallel_map_items(
        ...
        extra_args=[outdir, as_cog, te_dems])
...
def _item_map(stac_item, outdir, *args):
    ...
    if item.properties['constellation'] == 'worldview' and item.properties['nitf:image_preprocessing_level'] == '1R':
        output_stac_item = orthorectify(
            stac_item, item_outdir, **kwargs)
    else:
...
def pansharpen(stac_item_pan, stac_item_msi, outdir):
    '''
