...
def extract_features(checkpoint,  
    ...
    """Function for extracting features given kwcoco reference to data and annotations. Output is copy of kwcoco file with paths towards saved pixel-wise feature vectors.
...
#     dataset = kwcoco.CocoDataset(kwcoco)
    ...
    if not image_ids:
        image_ids = range(1,1+len(dataset.imgs)) ### include all available images if none are specified
