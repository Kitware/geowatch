...
def s2_coregister(mgrs_tile, input_folder, output_folder, baseline_scene=None):
    '''
    Coregister Sentinel-2 scenes in the same MGRS tile and save them to disk.

    Args:
        mgrs_tile: str
        input_folder: containing S2 safedirs to be coregistered
        output_folder: to save coregistered scenes in the same format as input_folder
        baseline_scene: pd.DataFrame or path to .csv containing the scene to use as baseline.
    ...
    if isinstance(baseline_scene, str):
        ...
        if not os.path.isfile(baseline_scene):
    ...
    pfname_list = glob.glob(input_folder +
                            '/*%s*.SAFE/GRANULE/%s/IMG_DATA/*_%s.jp2' %
                            (mgrs_tile, ref_scene_granule_id, base_band),
                            recursive=True)
    ...
    pfname_list = glob.glob(input_folder +
                            '/*%s*.SAFE/GRANULE/*/IMG_DATA/*_%s.jp2' %
                            (mgrs_tile, base_band),
                            recursive=True)
    ...
    for tile in sorted(db_master.keys()):
        ...
        for x in db[tile]:
            ...
            if x == db_master[tile]:
                print(
                    'This is a master scene - just copy/translate to GTiff %s'
                    % (fname_base))
                for b in S2_BANDS:
                ...
                s2_master_array = np.array(
                    s2_master_ds.GetRasterBand(1).ReadAsArray(),
                    dtype=np.uint16)
                ...
                s2_slave_array = np.array(
                    s2_slave_ds.GetRasterBand(1).ReadAsArray(),
                    dtype=np.uint16)
                ...
                if (len(res_coregistration) > 0):
                    ...
                    for g in res_coregistration:
                        ...
                        if ((array[4] < error_threshold) &
                            (not np.isnan(array[4])) &
