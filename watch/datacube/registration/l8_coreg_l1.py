...
def print_usage():
   print("Usage: python l8_coreg_l1.py mgrs_tile input_folder_safe output_folder s2_folder")
   return
...
def l8_coregister(mgrs_tile, input_folder, output_folder, s2_folder):
   ...
   if not (os.path.isfile(os.path.join(s2_folder, fname_ref_scene))):
       print('[ERROR]: file %s with baseline scene not found'%(os.path.join(s2_folder, fname_ref_scene)))
       sys.exit(1)
   ...
   pfname_list = glob.glob(os.path.join(s2_folder,f'*{mgrs_tile}*.SAFE','GRANULE',ref_scene_granule_id,'IMG_DATA',f'*_{s2_base_band}.jp2'), recursive=True)
   ...
   pfname_list = glob.glob(os.path.join(input_folder,'LC*',f'*_{l8_base_band}.TIF'), recursive=True)
   ...
   for tile in sorted(db_master.keys()):
       ...
       for x in db.keys(): # x is scene id here
           ...
           s2_master_array = scipy.ndimage.zoom(s2_master_array, 1./3, order=3)
           ...
           com_prefix = (f'gdalwarp -overwrite -of VRT -r cubic -tr 30 30  '
           ...
           l8_slave_ds = gdal.Open(os.path.join(path_out_data, l8_fname_tmp_base_band))
           l8_slave_array = np.array(l8_slave_ds.GetRasterBand(1).ReadAsArray(), dtype=np.uint16)
           # since we are adding padding we take a subset
           l8_slave_array = l8_slave_array[padding_px:(-padding_px), padding_px:(-padding_px)]
           ...
           # Running GCP search
           res_coregistration = get_gcp_for_registration(s2_master_ds, l8_slave_ds, s2_master_array, l8_slave_array, w_size=64, w_step=64) # production mode
           ...
           if (len(res_coregistration)>0):
               ...
               for g in res_coregistration:
                   ...
                   if ((array[4] < error_threshold) & (not np.isnan(array[4])) & (abs(array[5])<max_shift_threshold) & (abs(array[6])<max_shift_threshold)):
                       ...
                       output_str_30 = "-gcp %s %s %.5f %.5f %.f " % (array[0]+padding_px,array[1]+padding_px,array[2]+30*padding_px,array[3]-30*padding_px,elev)
                       f_30.write(output_str_30)
                       # For 15 m, we multiply by 2 pixel/line
                       output_str_15 = "-gcp %.5f %.5f %.5f %.5f %.f " % (array[0]*2.+padding_px,array[1]*2.+padding_px,array[2]+30*padding_px,array[3]-30*padding_px,elev)
               ...
               f_meta_out = open(os.path.join(path_out_data, fname_meta_updated), 'w')
               with open(os.path.join(path_data, fname_meta)) as f_meta_in:
                   for line in f_meta_in:
                       ...
                       if (('FILE_NAME_BAND' in line) | ('FILE_NAME_QUALITY' in line) | ('METADATA_FILE_NAME' in line) |\
                           ...
                           line_update = line.replace(f'{scene_id}', f'{scene_id_mgrs}')
                       ...
                       if ('CORNER_LL_PROJECTION_Y_PRODUCT' in line):
                           tmp = line.split(' = ')
                           line_update = f'{tmp[0]} = {lr_y}\n'
                       if (('PANCHROMATIC_LINES' in line) | ('PANCHROMATIC_SAMPLES' in line)):
                           tmp = line.split(' = ')
                           line_update = f'{tmp[0]} = {int(2*xsize/3.)}\n'
                       if (('REFLECTIVE_LINES' in line) | ('REFLECTIVE_SAMPLES' in line) | ('THERMAL_LINES' in line) | ('THERMAL_SAMPLES' in line)):
                           tmp = line.split(' = ')
               ...
               copyfile(os.path.join(path_data, fname_meta), os.path.join(path_out_data, fname_meta))
               ...
               for b in L8_BANDS:
                   ...
                   com_prefix = (f'gdalwarp -overwrite -of VRT -r cubic -tr {abs(x_res)} {abs(y_res)} '
                   ...
                   os.system(f'{com_gdal_translate_prefix} {os.path.join(path_out_data, l8_fname_tmp_band)} {os.path.join(path_out_data, fname_vrt)}')
                   os.system(f'{com_gdalwarp_prefix} {os.path.join(path_out_data, fname_vrt)} {os.path.join(path_out_data, fname_out)}')
               ...
               for b in extra_bands:
                   ...
                   # convert band to MGRS tmp file
                   com_prefix = (f'gdalwarp -overwrite -of VRT -r {resampling_method} -tr {abs(x_res)} {abs(y_res)} '
                                 f'-te {x_min-abs(x_res)*padding_px} {y_min-abs(x_res)*padding_px} {x_max+abs(x_res)*padding_px} {y_max+abs(x_res)*padding_px} '
