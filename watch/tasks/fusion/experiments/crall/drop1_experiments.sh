...
SUBDATA_PATH=$DVC_DPATH/drop1-S2-L8-aligned-old
...
# Merge together team features
python -m watch.cli.coco_combine_features --src \
    $SUBDATA_PATH/data_gsd10.kwcoco.json \
    $SUBDATA_PATH/uky_invariants.kwcoco.json \
    --dst $SUBDATA_PATH/data_teamfeats_gsd10_2.kwcoco.json 


# Split out train and validation data (TODO: add test when we can)
kwcoco subset --src $SUBDATA_PATH/data_teamfeats_gsd10.kwcoco.json \
        --dst $SUBDATA_PATH/vali_teamfeats_gsd10.kwcoco.json \
        --select_videos '.name | startswith("KR_")'

kwcoco subset --src $SUBDATA_PATH/data_teamfeats_gsd10.kwcoco.json \
        --dst $SUBDATA_PATH/train_teamfeats_gsd10.kwcoco.json \
        --select_videos '.name | startswith("KR_") | not'

# Print stats
kwcoco stats \
    $SUBDATA_PATH/train_teamfeats_gsd10.kwcoco.json \
    $SUBDATA_PATH/vali_teamfeats_gsd10.kwcoco.json 
...
TRAIN_FPATH=$SUBDATA_PATH/train_teamfeats_gsd10.kwcoco.json
VALI_FPATH=$SUBDATA_PATH/vali_teamfeats_gsd10.kwcoco.json
TEST_FPATH=$SUBDATA_PATH/vali_teamfeats_gsd10.kwcoco.json
