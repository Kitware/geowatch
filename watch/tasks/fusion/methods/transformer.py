...
class ChannelwiseChangeDetector(pl.LightningModule):
    def __init__(self, 
                 window_size=8, 
                 embedding_size=128, 
                 n_layers=4, 
        ...
        layers = [
            # nn.Transformer* expect inputs shaped (sequence, batch, feature)
            Rearrange("b t c (h hs) (w ws) -> b t (ws hs) c h w",
                      hs=self.hparams.window_size, 
                      ws=self.hparams.window_size),
            AddPositionalEncoding(2, [1, 3, 4, 5]),
            Rearrange("b t f c h w -> b (t c h w) f"),
            nn.LazyLinear(embedding_size),
            Rearrange("b s f -> s b f"),
        ] + [
            nn.TransformerEncoderLayer(
    ...
    def forward(self, images):
        ...
        # similarity between neighboring timesteps
        feats = nn.functional.normalize(feats, dim=2)
        similarity = torch.einsum("b t f c h w , b t f c h w -> b t c h w", feats[:,:-1], feats[:,1:])
