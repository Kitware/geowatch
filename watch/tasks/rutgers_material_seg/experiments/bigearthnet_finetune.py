...
class Trainer(object):
    ...
    def train(self, epoch: int, cometml_experiemnt: object) -> float:
        ...
        for batch_index, batch in pbar:
            ...
            loss = F.multilabel_soft_margin_loss(output, label.float())
            ...
            # preds_inds = (output>0.5).nonzero()
            # preds_inds = output>0.5
            # preds1 = torch.zeros_like(label)
            # preds1[preds_inds] = 1
                ...
                if (epoch) % config['visualization']['visualize_training_every'] == 0:
                    if (batch_index % iter_visualization) == 0:
                        ...
                        if config['visualization']['titles']:
                            ...
                            ax4.set_title(f"pred_fp_tp_fn_prediction_mask", fontsize=config['visualization']['font_size'])
    ...
    def forward(self, cometml_experiment: object, world_size: int = 8) -> tuple:
        ...
        for epoch in range(0, self.epochs):
            ...
            if (train_loss <= best_train_loss) or (val_mAP >= best_val_mAP):

                if train_loss <= best_train_loss:
                    best_train_loss = train_loss
                if val_mAP >= best_val_mAP:
                    best_val_mAP = val_mAP
                # if val_mAP >= best_val_change_f1:
                #     best_val_change_f1 = val_mAP

                model_save_name = f"{current_path[-1]}_epoch_{epoch}_loss_{train_loss}_valmAP_{val_mAP}_time_{datetime.datetime.today().strftime('%Y-%m-%d-%H:%M:%S')}.pth"
