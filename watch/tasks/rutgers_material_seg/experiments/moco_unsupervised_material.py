...
def main_worker():
    ...
    model = models.__dict__[config['training']['model_name']]()
    ...
    if config['training']['pretrained']:
        if os.path.isfile(config['training']['pretrained']):
            print("=> loading checkpoint '{}'".format(config['training']['pretrained']))
            checkpoint = torch.load(config['training']['pretrained'], map_location="cpu")
            ...
            print("=> loaded pre-trained model '{}'".format(config['training']['pretrained']))
        else:
            print("=> no checkpoint found at '{}'".format(config['training']['pretrained']))
    ...
    optimizer = torch.optim.SGD(parameters, config['training']['learning_rate'],
    ...
    if config['training']['resume']:
        if os.path.isfile(config['training']['resume']):
            print("=> loading checkpoint '{}'".format(config['training']['resume']))
            # if args.gpu is None:
            ...
            #     checkpoint = torch.load(config['training']['resume'])
            config['training']['start_epoch'] = checkpoint['epoch']
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(config['training']['resume'], checkpoint['epoch']))
        else:
    ...
    train_loader = dataset.make_loader(batch_size=config['training']['batch_size'])
    val_loader = train_loader.copy()
    ...
    if not config['procedures']['train']:
