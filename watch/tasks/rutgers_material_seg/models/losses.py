...
class SoftSupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, soft_mask=None):
        """Compute loss for model. If both `labels` and `mask` are None,
        it degenerates to SimCLR unsupervised loss:
        https://arxiv.org/pdf/2002.05709.pdf
        ...
        batch_size = features.shape[0]
        if labels is not None and soft_mask is not None:
            raise ValueError('Cannot define both `labels` and `mask`')
        elif labels is None and soft_mask is None:
            soft_mask = torch.eye(batch_size, dtype=torch.float32).to(device)
        elif labels is not None:
            labels = labels.contiguous().view(-1, 1)
            if labels.shape[0] != batch_size:
                raise ValueError('Num of labels does not match num of features')
            soft_mask = torch.eq(labels, labels.T).float().to(device)
        else:
            soft_mask = soft_mask.float().to(device)
        ...
        exp_logits = torch.exp(logits) * logits_mask
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))
        # log_prob = logits - torch.nn.functional.log_softmax(logits, dim = 1)

        # compute mean of log-likelihood over positive
        mean_log_prob_pos = (soft_mask * log_prob).sum(1) / soft_mask.sum(1)
        # mean_log_prob_pos = (soft_mask * log_prob).sum(1) / logits.shape[0]
