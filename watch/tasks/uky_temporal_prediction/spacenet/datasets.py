...
class S7_sort(data.Dataset):
    ...
    def __init__(self,
        ...
        self.train=train
        self.crop = A.Compose([A.RandomCrop(height=crop_size[0], width=crop_size[1])], additional_targets={'image2': 'image'})
        if self.train:
        ...
        self.normalization = A.Normalize(mean=self.normalize_params[0], std=self.normalize_params[1], max_pixel_value=1.0)
    ...
    def __getitem__(self, idx):
        ...
        image = (tif.imread(self.images[idx])).astype("float32")[:,:,:3] / 255
        ...
        if self.display:
            return torch.tensor(image).permute(2,0,1), torch.tensor(image2).permute(2,0,1), date1, date2, torch.tensor(image_orig).permute(2,0,1), torch.tensor(image2_orig).permute(2,0,1)
        
        return torch.tensor(image).permute(2,0,1), torch.tensor(image2).permute(2,0,1), date1, date2
...
class S7_time_aligned(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_false = (tif.imread(self.images[idx2])).astype("float32")[:,:,:3] / 255
...
class S7_change_augmentations(data.Dataset):
    normalize_params = [[0.16198677, 0.22665408, 0.1745371], [0.06108317, 0.06515977, 0.04128775]] ### fix
    ...
    train = True
        ...
        self.train=train
        self.crop = A.Compose([A.RandomCrop(height=crop_size[0], width=crop_size[1])], additional_targets={'image2': 'image', 'mask2': 'mask'})
        if self.train:
            self.transforms = A.Compose([A.RandomRotate90(),
                                         A.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.),
                                        ],
    ...
    def __getitem__(self, idx):
        ...
        while abs(idx2 - idx) < 5:
            idx2 = torch.randint(len(os.listdir(im_directory)),(1,))[0]
        im2_path = os.path.join(im_directory, sorted(os.listdir(im_directory))[idx2])
        mask2_path = os.path.join(mask_directory, sorted(os.listdir(mask_directory))[idx2])
        
        image = (tif.imread(self.images[idx])).astype("float32") / 255
        image2 = (tif.imread(im2_path)).astype("float32") / 255
        buildings = tif.imread(self.masks[idx]).astype("int32") // 255 # buildings are encoded as 255, bring this down to 1
        buildings2 = tif.imread(mask2_path).astype("int32") // 255
        
        
        crop = self.crop(image=image, mask=buildings, image2=image2, mask2=buildings2)
        ...
        if self.transforms:
            transformed = self.transforms(image=image[:,:,0:3], image2 = image2[:,:,0:3], mask=buildings, mask2=buildings2)
        ...
        assert(mask2_path[-47:-43] + '-' + mask2_path[-42:-40] == im2_path[-47:-43] + '-' + im2_path[-42:-40])
        ...
        return (torch.tensor(image).permute(2,0,1), 
                torch.tensor(image2).permute(2,0,1), 
                torch.tensor(buildings), 
                torch.tensor(buildings2),
                date1,
                date2,
                region
               )
