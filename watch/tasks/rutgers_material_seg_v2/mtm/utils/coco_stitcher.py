...
class CocoStitchingManager(object):
    ...
    def __init__(self,
        ...
        if writer_queue is None:
            ...
            writer_queue = BlockingJobQueue(mode='serial', max_workers=0)
        self.writer_queue = writer_queue

        self.suffix_code = (self.chan_code
                            if '|' not in self.chan_code else ub.hash_data(self.chan_code)[0:16])
    ...
    def accumulate_image(self,
                         gid,
                         space_slice,
                         data,
                         asset_dsize=None,
                         scale_asset_from_stitchspace=None,
                         is_ready='auto',
                         **kwargs):
        """
        ...
        if kwargs.get('dsize', None) is not None:
            ...
            if 0:
                ub.schedule_deprecation('watch',
                                        'dsize',
                                        'arg of accumulate_image',
                                        'use asset_dsize instad',
                                        deprecate='now')
        ...
        if kwargs.get('scale', None) is not None:
            ...
            if 0:
                ub.schedule_deprecation('watch',
                                        'scale',
                                        'arg of accumulate_image',
                                        'use scale_asset_from_stitchspace instad',
                                        deprecate='now')
        ...
        if self.stiching_space == 'video':
            ...
            if gid not in self.image_stitchers:
                ...
                self.image_stitchers[gid] = kwarray.Stitcher(asset_dims, device=self.device)
    ...
    def _stitcher_center_weighted_add(stitcher, asset_space_slice, data):
        ...
        TODO: refactor
        """
        weights = upweight_center_mask(data.shape[0:2])
        ...
        if stitcher.shape[0] < asset_space_slice[0].stop or stitcher.shape[1] < asset_space_slice[
                1].stop:
        ...
        try:
            if (asset_data.shape[0] != asset_slice[0].stop) or (asset_data.shape[1] !=
                                                                asset_slice[1].stop):
                from einops import rearrange
                asset_data = rearrange(asset_data, 'h w c -> c h w')
                asset_data, _ = add_buffer_to_image(asset_data, asset_slice[0].stop,
                                                    asset_slice[1].stop)
                asset_data = rearrange(asset_data, 'c h w -> h w c')
    ...
    def finalize_image(self, gid):
        ...
        try:
            ...
            if gid in self._seen_gids:
                raise KeyError(('Attempted to finalize image gid={}, but we already '
                                'finalized it').format(gid))
            else:
                raise KeyError(
                    'Attempted to finalize image gid={}, but no data was ever accumulated for it'.
                    format(gid))
                raise KeyError(('Attempted to finalize image gid={}, but no data '
        ...
        if self.write_prediction_attrs:
            ...
            img['has_predictions'] = ub.dict_union(img.get('has_predictions', {}),
                                                   {self.chan_code: True})
        ...
        #         # Convert to polygons
        #         scored_polys = list(
        #             mask_to_polygons(probs=band_probs,
        #                              thresh=thresh,
        #                              scored=True,
        #                              use_rasterio=False))
        ...
        #             self.result_dataset.add_annotation(image_id=gid,
        #                                                category_id=cid,
        #                                                bbox=bbox,
...
def mask_to_polygons(probs, thresh, bounds=None, use_rasterio=True, thresh_hysteresis=None):
    """
    Args:
    ...
    polygons = kwimage.Mask(binary_mask, 'c_mask').to_multi_polygon(pixels_are=pixels_are)
...
def upweight_center_mask(shape):
    ...
    weights = kwimage.morphology(weights,
                                 kernel=kernel,
                                 mode='dilate',
                                 element='rect',
                                 iterations=1)
