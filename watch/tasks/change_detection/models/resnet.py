...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(4, self.inplanes, kernel_size=7, stride=self.strides[0], padding=3,
                               bias=False)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        # Need to modify the original weight to add depth channel
        org_shape = state_dict['conv1.weight'].shape
        tmp = torch.zeros(org_shape[0], 1, org_shape[2], org_shape[3])
        new_weight = torch.cat((state_dict['conv1.weight'], tmp), 1)
