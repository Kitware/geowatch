...
def cal_pca_matrix(path='PCA_matrix.mat', ksize=15,
                   l_max=12.0, dim_pca=15, num_samples=500):
...
def shifted_anisotropic_Gaussian(k_size=np.array([15, 15]), scale_factor=np.array([
...
def gen_kernel(k_size=np.array([25, 25]), scale_factor=np.array(
        [4, 4]), min_var=0.6, max_var=12., noise_level=0):
    """"
    # modified version of https://github.com/assafshocher/BlindSR_dataset_generator
    # Kai Zhang
    # min_var = 0.175 * sf  # variance of the gaussian kernel will be sampled between min_var and max_var
...
def imfilter(x, k):
    ...
    k: kernel, cx1xhxw
    '''
    x = pad_circular(x, padding=(
        (k.shape[-2] - 1) // 2, (k.shape[-1] - 1) // 2))
