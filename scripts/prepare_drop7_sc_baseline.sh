...
#### ------------
#### AC Check 2024-01-22, look at performance difference with new ac-refinement algorithms
...
python -m geowatch.mlops.schedule_evaluation --params="
    ...
    " \
    --root_dpath="$DVC_EXPT_DPATH/_ac_refinement_test" \
    --queue_name "_ac_refinement_test" \
    --devices="0,1" \
    --backend=tmux --tmux_workers=4 \
    --cache=1 --skip_existing=1 --run=1
...
python -m geowatch.mlops.aggregate \
    --pipeline=sc \
    --target "
        - $DVC_EXPT_DPATH/_ac_refinement_test
    " \
    --output_dpath="$DVC_EXPT_DPATH/_ac_refinement_test/aggregate" \
    --resource_report=1 \
    --eval_nodes="
        - sc_poly_eval
    " \
    --plot_params="
        ...
        params_of_interest:
            ...
            - resolved_params.sc_poly.smoothing
            - resolved_params.sc_poly.site_score_thresh
