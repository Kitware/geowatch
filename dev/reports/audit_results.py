...
def audit():
    ...
    pro_dmod = kwcoco_datamodule.KWCocoVideoDataModule(**dict(
        test_dataset=pro_input_dset.fpath, **dataset_params,
        use_grid_cache=False,
    ))
    dev_dmod = kwcoco_datamodule.KWCocoVideoDataModule(**dict(
        test_dataset=dev_input_dset_fpath, **dataset_params,
        use_grid_cache=False,
    ))
    pro_dmod.setup('test')
    dev_dmod.setup('test')

    len(pro_dmod.test_dataset.new_sample_grid['targets'])
    len(dev_dmod.test_dataset.new_sample_grid['targets'])
    ...
    (pro_sampler.affinity - dev_sampler.affinity).sum()

    is_diff = (dev_sampler.unixtimes - pro_sampler.unixtimes) > 0
    dev_sampler.video_gids[is_diff]
    pro_sampler.video_gids[is_diff]

    dates1 = dev_input_dset.videos().images[0].lookup('date_captured')
    dates2 = pro_input_dset.videos().images[0].lookup('date_captured')

    fx1 = dev_input_dset.videos().images[0].lookup('frame_index')
    ...
    bad_idxs = np.where((np.array(dates1) != np.array(dates2)))
    print(bad_idxs)
    ...
    # name = ub.peek(missing2)
    # pro_gid = pro_input_dset.index.name_to_img[name]['id']
    # dev_gid = dev_input_dset.index.name_to_img[name[:-1] + '1']['id']

    name = 'crop_20150203T020000Z_N37.643680E128.649453_N37.683356E128.734073_L8_0'
    pro_gid = pro_input_dset.index.name_to_img[name]['id']
    dev_gid = dev_input_dset.index.name_to_img[name]['id']
    coco_img1 = dev_input_dset.coco_image(dev_gid)
    coco_img2 = pro_input_dset.coco_image(pro_gid)
    ...
    ub.dict_hist(dev_input_dset.images().lookup('sensor_coarse'))
    ub.dict_hist(pro_input_dset.images().lookup('sensor_coarse'))
