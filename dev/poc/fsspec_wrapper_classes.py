...
def main_poc():
    local_root = LocalPath(ub.Path.appdir('watch/tests/ffspec/local_src').ensuredir())
    local_root.delete()
    local_root.ensuredir()

    # We will copy from the remote back to here.
    local_roundtrip_root = LocalPath(ub.Path.appdir('watch/tests/ffspec/local_roundtrip').ensuredir())
    ...
    remote_root = S3Path('s3://kitware-smart-watch-data/testing')
    local_dst_root1 = LocalPath(ub.Path.appdir('watch/tests/ffspec/dests/local_dst'))
    local_dst_root2 = LocalPath(ub.Path.appdir('watch/tests/ffspec/dests/nested/local/dst'))
