...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    """
    if not (torch.is_tensor(tensor) or (isinstance(tensor, list)
...
def paired_random_crop(img_gts, img_lqs, gt_patch_size, scale, gt_path=None):
    ...
    if input_type == 'Tensor':
        ...
        img_gts = [v[top_gt:top_gt +
                     gt_patch_size, left_gt:left_gt +
                     gt_patch_size, ...] for v in img_gts]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
...
class LmdbBackend(BaseStorageBackend):
    ...
    def __init__(self, db_paths, client_keys='default',
                 readonly=True, lock=False, readahead=False, **kwargs):
        try:
            import lmdb
        except ImportError:
