...
def get_uperleft_denominator(img, kernel):
    ...
    upperleft = np.expand_dims(np.conj(V), axis=2) * \
        np.fft.fft2(img, axes=[0, 1])
...
# otf2psf: not sure where I got this one from. Maybe translated from
# Octave source code or whatever. It's just math.
def otf2psf(otf, outsize=None):
    ...
    if not isinstance(outsize, type(None)):
        insize = np.array(otf.shape)
...
def wrap_boundary(img, img_size):
    ...
    # r_A(alpha+1:end-alpha, end) = (1-a)*r_A(alpha,end) +
    # a*r_A(end-alpha+1,end)
    ...
    if alpha == 1:
        ...
        C2 = solve_min_laplacian(
            r_C[alpha - 1:-alpha + 1, alpha - 1:-alpha + 1])
...
def solve_min_laplacian(boundary_image):
    ...
    f_bp = np.zeros((H, W))
    f_bp[np.ix_(j,
                k)] = -4 * boundary_image[np.ix_(j,
                                                 k)] + boundary_image[np.ix_(j,
                                                                             k + 1)] + boundary_image[np.ix_(j,
                                                                                                             k - 1)] + boundary_image[np.ix_(j - 1,
                                                                                                                                             k)] + boundary_image[np.ix_(j + 1,
                                                                                                                                                                         k)]
    ...
    del (f3)
    if tt.shape[1] == 1:
        img_tt = np.transpose(fftpack.idst(np.transpose(
            tt) * 2, type=1) / (2 * (tt.shape[0] + 1)))
    else:
        img_tt = np.transpose(fftpack.idst(np.transpose(
            tt) * 2, type=1, axis=0) / (2 * (tt.shape[1] + 1)))
    del (tt)
